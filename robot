...
function xdot=robot(t,x,ths,spec,Kpid)
xdot=zeros(8,1);
%% set-points
th1s=ths(1);
th2s=ths(2);
...
Bq=[b11 b12;b21 b22];
%% C Matrix
c1=-M2*L1*(L2/2)*sin(x(4))*(2*x(5)*x(6)+x(6)^2);
c2=M2*L1*(L2/2)*sin(x(4))*x(5)*x(5);
Cq=[c1;c2];
%% Gravity Matrix
g1=(M1*(L1/2)+M2*L2)*g*cos(x(3))+M2*g*(L2/2)*cos(x(3)+x(4));
g2=M2*g*(L2/2)*cos(x(3)+x(4));
Gq=[g1;g2];
%% PID Control
