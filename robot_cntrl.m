close all
clear all
clc
%% Initilization
th_int=[0 0]; %initial positions
ths=[pi/4 pi/4]; %set-points
x0=[0 0 th_int 0 0 0 0]; %states initial values
Ts=[0 1]; %time span
%% Robot Specifications
L1=1; %link 1
L2=1; %link 2
M1=1.25; %mass 1
M2=1.5; %mass 2
spec=[L1 L2 M1 M2];
%% PID Parameters
% PID parameters for theta 1
Kp1=1;
Kd1=3;
Ki1=2;
% PID parameters for theta 2
Kp2=1;
Kd2=3;
Ki2=2;
Kpid=[Kp1 Kd1 Ki1 Kp2 Kd2 Ki2];
%% ODE solving
% opt1=odeset('RelTol',1e-10,'AbsTol',1e-20,'NormControl','off');
[T,X] = ode45(@(t,x) robot(t,x,ths,spec,Kpid),Ts,x0);
%% Output
th1=(3*pi/4)*(T.^2)-(pi/2)*(T.^3); %theta1 wavwform
th2=(3*pi/4)*(T.^2)-(pi/2)*(T.^3); %theta2 wavwform
%torque inputs computation from the 7th,8th states inside ODE
F1=diff(X(:,7))./diff(T);
F2=diff(X(:,8))./diff(T);
tt=0:(T(end)/(length(F1)-1)):T(end);
%xy
x1=(L1/2).*sin(th1); % X1
y1=(L1/2).*cos(th1); % Y1
x2=L1.*sin(th1)+(L2/2).*sin(th1+th2); % X2
y2=L1.*cos(th1)+(L2/2).*cos(th1+th2); % Y2
xee=L1.*sin(th1)+L2.*sin(th1+th2);
yee=L1.*cos(th1)+L2.*cos(th1+th2);

%theta1 error plot
plot(T,ths(1)-th1)
grid
title('Theta-1 error')
ylabel('theta1 error (rad)')
xlabel('time (sec)')
%theta2 error plot
figure
plot(T,ths(2)-th2)
grid
title('Theta-2 error')
ylabel('theta2 error (rad)')
xlabel('time (sec)')
%torque1 plot
figure
plot(tt,F1)
grid
title('Torque of theta 1')
ylabel('theta1 torque')
xlabel('time (sec)')
%torque2 plot
figure
plot(tt,F2)
grid
title('Torque of theta 2')
ylabel('theta2 torque')
xlabel('time (sec)')
